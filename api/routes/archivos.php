 ...
 *
 *  Sube y guarda archivos REALES (PDF, Word, imagenes) asociados a una causa.
 *
 *  DONDE SE GUARDAN (seguro):
 ...
 *      /home/.../<tu-dominio>/apice_archivos/<estudio>/<causa>/<archivo>
    ...
    *    Todo se separa por ESTUDIO. El estudio SIEMPRE se toma de la sesion de la
    *    persona logueada (nunca de lo que manda el navegador), asi un estudio no
    *    puede ver archivos de otro. La "causa" es un identificador interno de la
    *    app (modo rapido: las causas viven en el bloque de estado, no en tablas),
    ...
    *    PUT    /api/archivos/{id}            -> renombrar / cambiar visibilidad / fecha
    *    DELETE /api/archivos/{id}            -> eliminar (borra el archivo del disco)
    *
    *  FECHA DEL DOCUMENTO (fecha_doc):
    *    Es la fecha REAL del documento (no la fecha en que se subio el archivo).
    *    Sirve para cargar documentos viejos y que la lista quede ordenada por
    *    la cronologia real del expediente, no por el orden de carga.
    * ========================================================================== */
...
/* Valida una fecha en formato YYYY-MM-DD (la que manda un input de fecha). */
function archivos_fecha_ok($s) {
    $s = trim((string)$s);
    if ($s === '') return null;
...
/* Crea la tabla de archivos si todavia no existe (auto-instala). */
function asegurar_tabla_archivos() {
    ...
    /* Migracion: si una version previa creo causa_id como entero, pasarlo a texto. */
    try {
        ...
        /* Migracion: agregar la columna carpeta si la tabla ya existia sin ella. */
        try {
            ...
            /* Migracion: agregar la columna fecha_doc si la tabla ya existia sin ella. */
            try {
                  $col = db()->query("SHOW COLUMNS FROM archivos LIKE 'fecha_doc'")->fetch();
                  if (!$col) {
                          db()->exec("ALTER TABLE archivos ADD COLUMN fecha_doc DATE NULL AFTER visible_cliente");
                  }
            } catch (Throwable $e) { /* silencioso */ }
}
...
function archivo_subir() {
    ...
    if ($causaId === '') json_error('Falta indicar la causa.');
        ...
        if (empty($_FILES['file']) || !isset($_FILES['file']['error']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
              json_error('No se recibio el archivo (puede ser demasiado grande).');
        }
            $f = $_FILES['file'];
    $orig = (string)$f['name'];
    $size = (int)$f['size'];
    if ($size <= 0) json_error('El archivo esta vacio.');
        if ($size > archivos_max_bytes()) json_error('El archivo supera el limite de 20 MB.');
    ...
    $fechaDoc = archivos_fecha_ok($_POST['fecha_doc'] ?? '') ?: date('Y-m-d');

    db()->prepare('INSERT INTO archivos (estudio_id, causa_id, carpeta, nombre, archivo, tipo, tamano, visible_cliente, fecha_doc, subido_por)
                     VALUES (?,?,?,?,?,?,?,?,?,?)')
            ->execute([$estudioId, $causaId, $carpeta, $nombre, $stored, $ext, $size, $visible, $fechaDoc, $u['id']]);
    json_ok(['id' => (int)db()->lastInsertId(), 'nombre' => $nombre, 'fecha_doc' => $fechaDoc], 201);
}

/* Listar archivos de una causa (siempre acotado al estudio de la persona), en
     orden cronologico real (por fecha_doc; si un archivo viejo no la tiene
...
function archivo_descargar($id) {
    ...
    if ($u['rol'] === 'cliente') {
          if ((int)$a['visible_cliente'] !== 1) json_error('No tenes acceso a este archivo.', 403);
          ...
          if (!$lk->fetch()) json_error('No tenes acceso a este archivo.', 403);
    }

        $path = archivos_base() . '/' . (int)$a['estudio_id'] . '/' . archivos_causa_safe($a['causa_id']) . '/' . $a['archivo'];
    if (!is_file($path)) json_error('El archivo ya no esta en el servidor.', 404);
...
/* Renombrar / cambiar visibilidad / cambiar fecha del documento. */
    ...
    if (field('nombre', '__NO__') !== '__NO__') {
        ...
        if (field('visible_cliente', '__NO__') !== '__NO__') {
            ...
            if (field('carpeta', '__NO__') !== '__NO__') {
                ...
                if (field('fecha_doc', '__NO__') !== '__NO__') {
                      $fd = archivos_fecha_ok(field('fecha_doc'));
                      if ($fd) { $sets[] = 'fecha_doc = ?'; $vals[] = $fd; }
                }
