...
/* Cambiar el tipo de un estudio (individual <-> estudio). Solo super-admin. */
function estudio_editar($id) {
  ...
  db()->prepare('UPDATE estudios SET tipo = ? WHERE id = ?')->execute([$tipo, $id]);
  json_ok(['actualizado' => true, 'tipo' => $tipo]);
}
