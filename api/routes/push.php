...
/* ============================================================================
* PUSH (/api/push/...)
* GET /api/push/clave -> clave publica VAPID (para suscribirse)
* POST /api/push/subscribe -> guarda la suscripcion del dispositivo
* GET /api/push/pendiente -> que aviso mostrar (lo consulta el celular)
* GET /api/push/probar -> envia un push de prueba a MIS dispositivos
* ========================================================================== */

require_once __DIR__ . '/../lib/push.php';
require_once __DIR__ . '/../lib/caducidad.php';
...
function handle_push($method, $resto) {
  ...
  json_error('Accion de push no valida.', 404);
}
...
function push_subscribe() {
  ...
  if ($endpoint === '' || stripos($endpoint, 'http') !== 0) json_error('Suscripcion invalida.');
...
/* Que mostrar cuando llega un push: audiencias/citas de hoy y manana, y
  causas con plazo de caducidad de instancia vencido o por vencer. */
...
$venc = causas_por_vencer($eid, 15);

$partes = [];
  if ($rows) {
    if (count($rows) === 1) {
      $r = $rows[0];
      $cuando = $r['fecha'] === $hoy ? 'hoy' : 'manana';
      $que = $r['tipo'] === 'cita' ? ('Cita con ' . ($r['cliente_nombre'] ?: 'un cliente')) : 'Audiencia';
      $partes[] = $que . ' ' . $cuando . ($r['hora'] ? (' a las ' . $r['hora'] . ' hs') : '') . '.';
    } else {
      $partes[] = 'Tenes ' . count($rows) . ' audiencias/citas para hoy y manana.';
    }
  }
  $txtCad = caducidad_texto($venc);
  if ($txtCad !== '') $partes[] = $txtCad;

if (!$partes) { json_ok(['title' => 'APICE', 'body' => 'Tenes novedades en tu estudio.']); return; }

$hayVencidas = $venc && $venc[0]['dias'] < 0;
  $title = $hayVencidas ? 'APICE - caducidad' : ($venc ? 'Recordatorio APICE' : 'Recordatorio APICE');
  json_ok(['title' => $title, 'body' => implode(' ', $partes)]);
}

/* Envia un push de prueba a los dispositivos del usuario logueado. */
